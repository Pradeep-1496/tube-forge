...
export class BackgroundManagementService {
  ...
  async update(
    ...
    const newType: BackgroundType = (type as BackgroundType) || background.type;
    const newName = name || background.name;
    ...
    if (file) {
      ...
      const targetDir =
        newType === BackgroundType.LANDSCAPE
          ? this.LANDSCAPE_DIR
          : this.PORTRAIT_DIR;
      if (!existsSync(targetDir)) {
        mkdirSync(targetDir, { recursive: true });
      ...
      background.path = join(
        newType === BackgroundType.LANDSCAPE ? 'landscape' : 'protrait',
        finalName,
      );
      background.size = statSync(targetPath).size;
    } else if (type && (type as BackgroundType) !== background.type) {
      const oldPath = join(
        process.cwd(),
        'assets',
        'backgrounds',
      ...
      const ext = this.getExtension(background.path);
      const sanitizedName = this.sanitizeFilename(background.name);
      const targetDir =
        newType === BackgroundType.LANDSCAPE
          ? this.LANDSCAPE_DIR
          : this.PORTRAIT_DIR;
